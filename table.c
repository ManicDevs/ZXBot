...
static void toggle_obf(uint8_t id)
{
	...
	*/
	...
	util_strxor(val->val, val->val, val->val_len);
...
void table_lock_val(uint8_t id)
{
	...
	if(val->locked)
	{
		util_msgc("Info", "Tried to double-lock value %d!", id);
		return;
	}
#endif
...
void table_unlock_val(uint8_t id)
{
    ...
    if (!val->locked)
    {
        util_msgc("Info", "Tried to double-unlock value %d!", id);
        return;
    }
#endif
...
char *table_retrieve_val(int id, int *len)
{
    ...
    if (val->locked)
    {
        util_msgc("Info", "Tried to access table.%d but it is locked!", id);
        return 0;
    }
#endif
